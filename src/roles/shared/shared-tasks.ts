...
export class SharedTasks {
  ...
  public static depositTask(creep: Creep, targets: AnyStructure[]): boolean {
    if (targets.length > 0) {
  ...
  public static buildTask(creep: Creep, targets: ConstructionSite[]): boolean {
    if (targets.length > 0) {
      const sortedTargets = _.sortBy(targets, target => creep.pos.getRangeTo(target));
  ...
  public static upgradeTask(creep: Creep): boolean {
  ...
  public static moveToControllerOrSpawnTask(creep: Creep, room: Room): boolean {
    const roomController = creep.room.controller;
    if (roomController) {
      creep.moveTo(roomController, { visualizePathStyle: { stroke: "#ffffff" } });
